...
export default class SignPage extends Vue {
  ...
  async register(e: Event) {
    ...
    this.$axios
      .post(this.$config.internalApi + "accounts/" + this.$i18n.locale, {
        email: this.email,
      ...
      .then((response: any) => {
        if (response.data.success === true) {
          MattermostService.sendToMarketingChannel(this.$config.internalApi, this.email + " is registered to app.")
          this.updateText(this.$i18n.t("signin.success"));
