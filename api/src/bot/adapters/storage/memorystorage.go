...
const (
	chunkSize              int    = 10000
	topKeywords            int    = 5
	thresholdForKeywords   int    = 5
	maxSearchResults       int    = 100
	thresholdForStopWords  int    = 100
	dictFile               string = "./etc/dict.txt"
	idfFile                string = "./etc/idf.txt"
	stopWordsFile          string = "./etc/stop_words.txt"
	generatedStopWordsFile string = "./etc/stopwords.txt"
)
...
func RestoreMemoryStorage(decoder *gob.Decoder) (*memoryStorage, error) {
	...
	absDictFile, _ := filepath.Abs(dictFile)
	absIdfFile, _ := filepath.Abs(idfFile)
	absStopWordsFile, _ := filepath.Abs(stopWordsFile)
	logx.Must(segmenter.LoadDictionary(absDictFile))
	var extracter analyse.TagExtracter
	logx.Must(extracter.LoadDictionary(absDictFile))
	logx.Must(extracter.LoadIdf(absIdfFile))
	logx.Must(extracter.LoadStopWords(absStopWordsFile))
...
func (storage *memoryStorage) saveStopWords() {
	absGeneratedStopWordsFile, _ := filepath.Abs(generatedStopWordsFile)
	f, err := os.Create(absGeneratedStopWordsFile)
	if err != nil {
		return
	}
	defer f.Close()
...
func (storage *memoryStorage) mapper(data interface{}, writer mr.Writer[any], cancel func(error)) {
	indexes := make(map[string][]int)
	chunk := data.(*keyChunk)
...
func (storage *memoryStorage) reducer(input <-chan interface{}, writer mr.Writer[any], cancel func(error)) {
